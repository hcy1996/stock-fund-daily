...
- GitHub runner 默认不保留本地 SQLite 和原始抓取数据，因此邮件发送日志与本地累计兜底历史不会自动跨天保留
...
- 同花顺：`20` 日页面抓取候选源，同时用于 Top10 概念成分股详情页
- 本地累计：当官方 `20 日累计` 和同花顺都不可用时，使用最近最多 `20` 个交易日的日资金流累计值

## 已知限制

- `20 日` 口径优先使用东方财富历史累计；若派生失败则退回同花顺页面，再退回本地累计口径
- 首次运行若本地历史不足 `20` 个交易日，`20 日` 区块会标记为“暖机中”
